      ...
      {tab === 'overview' ? <>
        ...
        <section className="risk-detail-grid">
          ...
          <article className="panel multi-trend-panel">
            ...
            <div className="multi-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={trend} margin={{ left: -25, right: 8, top: 10 }}><CartesianGrid stroke="rgba(151,172,199,.08)" vertical={false}/><XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill:'#718399',fontSize:10 }}/><YAxis domain={[35,90]} axisLine={false} tickLine={false} tick={{ fill:'#718399',fontSize:10 }}/><Tooltip contentStyle={{ background:'#101e2e',border:'1px solid #294056',borderRadius:9 }}/><Area dataKey="africa" name="非洲" type="monotone" stroke="#ff647a" fill="transparent" strokeWidth={2}/><Area dataKey="asia" name="亚洲" type="monotone" stroke="#f5b84d" fill="transparent" strokeWidth={2}/><Area dataKey="americas" name="美洲" type="monotone" stroke="#6d8cff" fill="transparent" strokeWidth={2}/></AreaChart></ResponsiveContainer></div>
            <div className="regional-legend"><span><i style={{background:'#ff647a'}}/>非洲 <b>{latestTrend?.africa?.toFixed(1)??'—'}</b></span><span><i style={{background:'#f5b84d'}}/>亚洲 <b>{latestTrend?.asia?.toFixed(1)??'—'}</b></span><span><i style={{background:'#6d8cff'}}/>美洲 <b>{latestTrend?.americas?.toFixed(1)??'—'}</b></span></div>
          </article>
        </section>
